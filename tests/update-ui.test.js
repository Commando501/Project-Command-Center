...
import { parseSemver } from '../src/updater/version.js';
...
/**
 * The version the synthetic "available release" claims.
 *
 * Derived from the installed version rather than hard-coded. It used to be the
 * literal '4.1.0', which silently stopped being newer the moment the package
 * itself reached 4.1.0 — every banner and review test then failed, blocking
 * the release that caused it. Deriving it keeps these tests correct at any
 * future version.
 */
let newerVersion;
...
beforeAll(async () => {
  ...
  const { major, minor } = parseSemver(installedVersion);
  newerVersion = `${major}.${minor + 1}.0`;
  builtHtml = await readFile(build.path, 'utf8');
  releaseShell = reversionShell(builtHtml, { appVersion: newerVersion, repository: REPOSITORY });
}, 120000);
...
describe('the availability banner', () => {
  test('appears when a newer release exists, and installs nothing', async () => {
    const github = await createFakeGitHub({ version: newerVersion, shellHtml: releaseShell });
    const session = boot(builtHtml, { fetchImpl: github.fetchImpl });

    await waitFor(() => visible(session, 'updateBanner'));
    expect(text(session, 'updateBannerText')).toBe(`Project Command Center ${newerVersion} is available`);
  ...
  test('View Update opens the review panel without downloading', async () => {
    ...
    expect(text(session, 'reviewCandidateVersion')).toBe(newerVersion);
...
describe('installing an official update, served over https', () => {
  test('downloads, verifies, and offers the upgraded file', async () => {
    ...
    expect(text(session, 'resultNewVersion')).toBe(newerVersion);
  ...
  test('a tampered download is refused and produces nothing', async () => {
    const github = await createFakeGitHub({ version: newerVersion, shellHtml: releaseShell, corruptBytes: true });
    const session = boot(builtHtml, { fetchImpl: github.fetchImpl, url: SERVED_URL });
...
describe('opened from disk, where release assets cannot be read', () => {
  ...
  test('the banner still appears, using the API digest', async () => {
    const github = await createFakeGitHub({ version: newerVersion, shellHtml: releaseShell, blockAssetFetch: true });
    const session = boot(builtHtml, { fetchImpl: github.fetchImpl });
  ...
  test('the review panel offers Download Release and explains the schema check', async () => {
    ...
    expect(session.document.getElementById('downloadReleaseBtn').getAttribute('href'))
      .toContain(`Project-Command-Center-v${newerVersion}.html`);
  ...
  test('the three steps are spelled out before the first click', async () => {
    ...
    expect(steps[0]).toContain(`Project-Command-Center-v${newerVersion}.html`);
  ...
  test('the downloaded file then installs and verifies from disk', async () => {
    ...
    const file = new session.window.File([releaseShell], `Project-Command-Center-v${newerVersion}.html`, { type: 'text/html' });
    ...
    expect(text(session, 'reviewVerification')).toContain(`Verified against the official ${newerVersion} release`);
...
describe('manual update files', () => {
  const selectFile = async (session, bytes, name = `Project-Command-Center-v${newerVersion}.html`) => {
    const input = session.document.getElementById('updateFileInput');
